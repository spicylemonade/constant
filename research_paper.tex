...
Fekete's lemma \citep{fekete1923}. They also computed small-$n$ exact values
but did not give nontrivial asymptotic bounds.
...
$\gamma_2$ at rate $O(\sqrt{n\log n}/n)$; KPZ universality conjectures the
sharper rate $O(n^{-2/3})$.
...
lemma \citep{fekete1923}, the limit $\gamma_2 = \sup_n a_n / n = \lim_{n
\to \infty} a_n / n$ exists and satisfies $a_N / N \leq \gamma_2$ for all $N$.
Kingman's subadditive ergodic theorem \citep{kingman1968} provides the
stochastic generalization that underpins the almost-sure convergence.
...
Numba JIT compilation \citep{lam2015} with \texttt{prange} for embarrassingly
...
$0.792666$ \citep{heineman2024} at approximately $M \approx 127{,}000$ trials.
...
\begin{figure}[t]
  ...
  \citep{heineman2024}. The provable bound exceeds this at $M \approx 127{,}000$.}
...
$\gamma_2$ is conjectured to be $O(n^{-2/3})$ under KPZ universality
(Alexander \citep{alexander1994} proved the weaker rate
$O(\sqrt{n\log n}/n)$). Our
